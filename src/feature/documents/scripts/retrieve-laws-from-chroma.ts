...
async function retrieveLaws(query: string, limit: number = 5): Promise<LegalDocumentChunk[]> {
  try {
...
async function main() {
  try {
    ...
    laws.forEach((law, index) => {
      logger.info(`📌 ${index + 1}. ${law.title} (Similarity: ${(law.similarity! * 100).toFixed(1)}%)`);
      logger.info(`   📚 Category: ${law.category}`);
      logger.info(`   📜 Source: ${law.source}`);
      logger.info(`   📝 ${law.content.substring(0, 150)}${law.content.length > 150 ? '...' : ''}\n`);
    });
