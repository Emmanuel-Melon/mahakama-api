...
import { PaginationOptions, PaginationParams, PaginationResult, SortDirection, SortConfig, SortConfigOptions, GetRequestQuery, BaseFilterParams } from './express.types';
...
export const applySearchConditions = (options?: GetRequestQuery, query: {
  schema: any,
  searchQuery: any,
  ...
  const searchFilters = fields.map(field => ilike(schema[query.field], formattedSearchQuery))
  if (search) {
    return (
      or(
        searchFilters,
